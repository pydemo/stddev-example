...
if 1:
	...
	async def merge_sums(counts_queue):
		total = 0
		totals = 0
		while True: 
	...
	async def s3_get_mean():
		queue = asyncio.Queue()
		counts_queue = asyncio.Queue()
		
		
		await asyncio.gather( get_files_to_scan(objs, queue), sums_in_table(queue, counts_queue), merge_sums(counts_queue))	
		await queue.join()
